...
def process_dataset(f):
    ...
    try:
        ...
        have_precursors, matched2 = explain_a_dataset_byMS2(
            all_representative_features, ms2_tree_pos, rt_stdev=dict_rtwindow[f]
        )
        delta_values_ms2 = []
        for x in matched2:
        ...
        with open(f'output_pos/{f}.json', 'w', encoding='utf-8') as json_file:
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_dataset, f): f for f in pos_orbi_datasets}
