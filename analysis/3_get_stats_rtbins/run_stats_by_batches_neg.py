...
def explain_a_dataset_byMS2(all_features, ms2_tree, rt_stdev, limit_ppm=5):
    ...
    for ff in all_features:
        ...
        in_rtwindow_mz = [x['mz'] for x in in_rtwindow if x['mz'] < ff['mz']*(1+limit_ppm*1e-6)]
        ...
        for sp in precursors:
            matched_ms2_mzs = find_match_ms2_from_mzs_in_rtbin(in_rtwindow_mz, [x[1] for x in sp['peaks']], limit_ppm)
            
            if matched_ms2_mzs:
                _deltas = [sp['mz']-x for x in matched_ms2_mzs] # delta between ms2 peaks and precursors
                matched.append((ff['id_number'], sp['name'], _deltas))
...
def process_dataset(f):
    ...
    try:
        ...
        have_precursors, matched2 = explain_a_dataset_byMS2(
            all_representative_features, ms2_tree_neg, rt_stdev=dict_rtwindow[f]
        )
        delta_values_ms2 = []
        for x in matched2:
        ...
        with open(f'output_neg/{f}.json', 'w', encoding='utf-8') as json_file:
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_dataset, f): f for f in neg_orbi_datasets}
